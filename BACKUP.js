...
app.get("/", function(request, response){
    ...
    let result = neededTags.filter(el => el === neededPosts)
    ...
    for (let i = 0; i < posts.length; i++) {
        ...
        if (tager.length > 0 ) {
            let searchedId = posts[i].id;
            tagsPosts.push(searchedId);
        }
    };


    const filteredPosts = posts.slice().filter(item => item.id == tagsPosts);

    console.log('This posts BLYAT==' + JSON.stringify(filteredPosts))
